...
from fraudGT.encoder.raw_encoder import (RawNodeEncoder, RawEdgeEncoder)
from fraudGT.encoder.hetero_raw_encoder import (HeteroRawNodeEncoder, HeteroRawEdgeEncoder)

from fraudGT.encoder.homo_gnn_encoder import HomoGNNEncoder
from fraudGT.encoder.hetero_gnn_encoder import HeteroGNNEncoder
from fraudGT.encoder.hetero_label_encoder import HeteroLabelNodeEncoder, HeteroLabelEdgeEncoder
from fraudGT.encoder.hetero_pos_encoder import (RWSENodeEncoder, Node2VecNodeEncoder, \
...
def concat_edge_encoders(encoder_classes, pe_enc_names):
    ...
    Args:
        encoder_classes: List of edge encoder classes
        pe_enc_names: List of PE embedding Encoder names, used to query a dict
            with their desired PE embedding dims. That dict can only be created
            during the runtime, once the config is loaded.
    ...
    class Concat3EdgeEncoder(torch.nn.Module):
        """Encoder that concatenates three edge encoders.
        """
        enc1_cls = None
        enc2_cls = None
        enc2_name = None
    ...
    if len(encoder_classes) == 2:
        ...
        Concat3EdgeEncoder.enc1_cls = encoder_classes[0]
        Concat3EdgeEncoder.enc2_cls = encoder_classes[1]
        Concat3EdgeEncoder.enc3_cls = encoder_classes[2]
        Concat3EdgeEncoder.enc2_name = pe_enc_names[0]
        Concat3EdgeEncoder.enc3_name = pe_enc_names[1]
        return Concat3EdgeEncoder
    else:
        raise ValueError(f"Does not support concatenation of "
                         f"{len(encoder_classes)} encoder classes.")

# Dataset-specific node encoders.
ds_encs = {'Raw': RawEdgeEncoder,
           'Hetero_Raw': HeteroRawEdgeEncoder,
          }

# Positional Encoding node encoders.
pe_encs = {
           'Hetero_Label': HeteroLabelEdgeEncoder,
          }
